...
cd build
../tar/gcc-5.1.0/configure --prefix=$OUT --enable-languages=c,c++  1> /dev/null
make -j4 all-gcc 1> /dev/null
make install-gcc 1> /dev/null
cd ..

cd glibc
../tar/glibc-2.21/configure --prefix=$OUT/$TARGET --build=$MACHTYPE --with-headers=$OUT/$TARGET/include  libc_cv_forced_unwind=yes 1> /dev/null
