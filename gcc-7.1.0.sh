...
cd tar
set +x
[ ! -f ./gcc-7.1.0.tar.gz ]     && wget https://ftp.gnu.org/gnu/gcc/gcc-7.1.0/gcc-7.1.0.tar.gz
[ ! -f ./binutils-2.28.tar.gz ]   && wget http://ftpmirror.gnu.org/binutils/binutils-2.28.tar.gz
[ ! -f ./linux-4.7.5.tar.xz ]   && wget https://www.kernel.org/pub/linux/kernel/v4.x/linux-4.7.5.tar.xz
[ ! -f ./glibc-2.25.tar.xz ]    && wget http://ftpmirror.gnu.org/glibc/glibc-2.25.tar.xz
...
[ ! -d ./gcc-7.1.0 ]        && tar xf gcc-7.1.0.tar.gz
[ ! -d ./binutils-2.28 ]    && tar xf binutils-2.28.tar.gz
[ ! -d ./linux-4.7.5 ]      && tar xf linux-4.7.5.tar.xz
[ ! -d ./glibc-2.25 ]       && tar xf glibc-2.25.tar.xz
...
cd build-binutils
../binutils-2.28/configure --prefix=$OUT --target=$TARGET --disable-nls --with-system-zlib 1> /dev/null
...
cd build
../tar/gcc-7.1.0/configure --prefix=$OUT --enable-languages=c,c++ --enable-multilib 1> /dev/null
make -j$THREADS all-gcc 1> /dev/null
make install-gcc 1> /dev/null
cd ..

cd glibc
../tar/glibc-2.25/configure --prefix=$OUT/$TARGET --build=$MACHTYPE --with-headers=$OUT/$TARGET/include  libc_cv_forced_unwind=yes --enable-multilib  1> /dev/null
