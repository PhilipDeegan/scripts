...
OUT=/media/philix/350GBEXT4/gcc_4.9_ARMv7-A
rm -rf build glibc $OUT
mkdir -p build glibc tar $OUT
cd tar
set +x
if [ ! -f ./gcc-4.9.2.tar.bz2 ]; then wget https://ftp.gnu.org/gnu/gcc/gcc-4.9.2/gcc-4.9.2.tar.bz2; fi
...
if [ ! -d ./gcc-4.9.2 ]; then tar xf gcc-4.9.2.tar.bz2; fi
...
cd build
PATH=$OUT/bin:$PATH ../tar/gcc-4.9.2/configure --prefix=$OUT --enable-languages=c,c++ --target=$TARGET --disable-nls --with-arch=armv7-a 1> /dev/null
