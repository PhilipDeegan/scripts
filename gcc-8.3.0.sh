...
set -x
cd ${LINUX}
make SHELL='sh -xv' CFLAGS="${CFG}" INSTALL_HDR_PATH=$OUT/$TARGET headers_install 1> /dev/null
cd ..
...
make SHELL='sh -xv' CFLAGS="${CFG}" -j$THREADS 1> /dev/null
make SHELL='sh -xv' CFLAGS="${CFG}" install 1> /dev/null
cd ..
...
make SHELL='sh -xv' CFLAGS="${CFG}" -j$THREADS all-gcc 1> /dev/null
make SHELL='sh -xv' CFLAGS="${CFG}" install-gcc 1> /dev/null
cd ..
...
make SHELL='sh -xv' CFLAGS="${CFG}" install-bootstrap-headers=yes install-headers 1> /dev/null
make SHELL='sh -xv' CFLAGS="${CFG}" -j$THREADS csu/subdir_lib 1> /dev/null
...
cd build
make SHELL='sh -xv' CFLAGS="${CFG}" -j$THREADS all-target-libgcc 1> /dev/null
make SHELL='sh -xv' CFLAGS="${CFG}" install-target-libgcc 1> /dev/null
cd ..
