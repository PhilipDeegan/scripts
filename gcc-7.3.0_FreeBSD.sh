...
TARGET=x86_64-freebsd11.1
OUT=/opt/chain/gcc7.3.0_freebsd11.1
THREADS=$(nproc --all)
...
cd build-binutils
../binutils-2.29/configure --prefix=$OUT --target=$TARGET --disable-nls --enable-libssp --enable-gold --enable-ld 1> /dev/null
...
cd build
PATH=$OUT/bin:$PATH ../tar/gcc-7.3.0/configure --prefix=$OUT --enable-languages=c,c++ --target=$TARGET --disable-nls --without-headers --with-gnu-as --with-gnu-ld --enable-languages=c,c++ --disable-nls --enable-libssp --enable-gold --enable-ld 1> /dev/null
