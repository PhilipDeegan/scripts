...
cd build-binutils
../binutils-2.25/configure --prefix=$OUT --target=$TARGET 1> /dev/null
make -j6 1> /dev/null
...
cd build
PATH=$OUT/bin:$PATH ../tar/gcc-5.1.0/configure --prefix=$OUT --enable-languages=c,c++ --target=$TARGET 1> /dev/null
PATH=$OUT/bin:$PATH make -j6 all-gcc 1> /dev/null
PATH=$OUT/bin:$PATH make install-gcc 1> /dev/null
cd ..

cd glibc
PATH=$OUT/bin:$PATH ../tar/glibc-2.21/configure --prefix=$OUT/$TARGET --build=$MACHTYPE --host=$TARGET --with-headers=$OUT/$TARGET/include libc_cv_forced_unwind=yes 1> /dev/null
