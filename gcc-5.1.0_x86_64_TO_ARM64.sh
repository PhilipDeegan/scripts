...
cd linux-4.1.1
make ARCH=arm64 INSTALL_HDR_PATH=$OUT/aarch64-linux headers_install
cd ..

mkdir build-binutils
cd build-binutils
../binutils-2.25/configure --prefix=$OUT --target=aarch64-linux --disable-multilib
make -j6
make install
cd ..
...
cd build
PATH=$OUT/bin:$PATH ../tar/gcc-5.1.0/configure --prefix=$OUT --enable-languages=c,c++ --disable-multilib --target=aarch64-linux
PATH=$OUT/bin:$PATH make -j6 all-gcc
PATH=$OUT/bin:$PATH make install-gcc
cd ..

cd glibc
PATH=$OUT/bin:$PATH ../tar/glibc-2.21/configure --prefix=$OUT/aarch64-linux --build=$MACHTYPE --host=aarch64-linux --target=aarch64-linux --with-headers=$OUT/aarch64-linux/include --disable-multilib libc_cv_forced_unwind=yes
PATH=$OUT/bin:$PATH make install-bootstrap-headers=yes install-headers
PATH=$OUT/bin:$PATH make -j6 csu/subdir_lib
mkdir $OUT/aarch64-linux/lib
PATH=$OUT/bin:$PATH install csu/crt1.o csu/crti.o csu/crtn.o $OUT/aarch64-linux/lib
PATH=$OUT/bin:$PATH aarch64-linux-gcc -nostdlib -nostartfiles -shared -x c /dev/null -o $OUT/aarch64-linux/lib/libc.so
touch $OUT/aarch64-linux/include/gnu/stubs.h
cd ..
