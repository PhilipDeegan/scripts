...
OUT=/opt/chain/gcc4.8.5
THREADS=$(nproc --all)
rm -rf build glibc $OUT
mkdir -p build glibc tar $OUT
cd tar
set +x
[ ! -f ./gcc-4.8.5.tar.gz ]     && wget https://ftp.gnu.org/gnu/gcc/gcc-4.8.5/gcc-4.8.5.tar.gz
...
[ ! -d ./gcc-4.8.5 ]     && tar xf gcc-4.8.5.tar.gz
...
cd build
../tar/gcc-4.8.5/configure --prefix=$OUT --enable-languages=c,c++ --enable-multilib 1> /dev/null
