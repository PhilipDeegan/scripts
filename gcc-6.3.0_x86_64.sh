...
cd tar
set +x
[ ! -f ./gcc-6.3.0.tar.gz ]     && wget https://ftp.gnu.org/gnu/gcc/gcc-6.3.0/gcc-6.3.0.tar.gz
...
[ ! -d ./gcc-6.3.0 ]        && tar xf gcc-6.3.0.tar.gz
...
cd build
../tar/gcc-6.3.0/configure --prefix=$OUT --enable-languages=c,c++ --enable-multilib 1> /dev/null
