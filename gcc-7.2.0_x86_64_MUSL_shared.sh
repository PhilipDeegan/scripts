...
PREFIX=/opt/chain/gcc7.2.0_SHMUSL
THREADS=8
...
mkdir build-musl
cd build-musl
CROSS_COMPILE=" " ../tar/musl*/configure --prefix="$PREFIX" --target="$ARCH"   1>/dev/null
...
mkdir build-gcc
cd build-gcc
../tar/gcc-7.2.0/configure --prefix="$PREFIX" --target="$TARGET" --with-sysroot="$PREFIX" --disable-bootstrap --disable-werror   --disable-multilib --disable-libsanitizer --enable-languages=c,c++ 1>/dev/null
make -j$THREADS 1>/dev/null
make install 1>/dev/null
cd ..
rm -rf build-gcc
...
for f in $(find ${PREFIX}/bin/ -name "amd64-linux-musl-*"); do D=$(dirname $f); F=$(basename $f); ln -fs $D/$F $D/${F:17};  done
...
mkdir build-musl
cd build-musl
CROSS_COMPILE="$TARGET-" ../tar/musl*/configure --prefix="$PREFIX" --target="$ARCH"   --syslibdir="$PREFIX/lib"  1>/dev/null
make -j$THREADS 1>/dev/null
make install 1>/dev/null
cd ..
rm -rf build-musl
...
mkdir build-gcc
cd build-gcc
../tar/gcc-7.2.0/configure --prefix="$PREFIX" --target="$TARGET" --with-sysroot="$PREFIX" --disable-multilib --disable-bootstrap --disable-werror  --enable-languages=c,c++ --disable-libsanitizer --libexecdir="$PREFIX/lib" 1>/dev/null
make -j$THREADS 1>/dev/null
make install 1>/dev/null
cd ..
rm -rf build-gcc
