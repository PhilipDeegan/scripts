...
OUT=/opt/chain/gcc8.1.0
THREADS=6
rm -rf build glibc $OUT
mkdir -p build glibc tar $OUT
cd tar
set +x
[ ! -f ./gcc-8.1.0.tar.gz ]     && wget https://ftp.gnu.org/gnu/gcc/gcc-8.1.0/gcc-8.1.0.tar.gz
...
[ ! -d ./gcc-8.1.0 ]     && tar xf gcc-8.1.0.tar.gz
...
cd build-binutils
../binutils-2.29/configure --prefix=$OUT --target=$TARGET --disable-nls --with-system-zlib  --without-selinux 1> /dev/null
...
cd build
../tar/gcc-8.1.0/configure --prefix=$OUT --enable-languages=c,c++ --enable-multilib --without-selinux 1> /dev/null
make -j$THREADS all-gcc 1> /dev/null
make install-gcc 1> /dev/null
cd ..

cd glibc
../tar/glibc-2.27/configure --prefix=$OUT/$TARGET --build=$MACHTYPE --with-headers=$OUT/$TARGET/include --without-selinux libc_cv_forced_unwind=yes --enable-multilib  1> /dev/null
